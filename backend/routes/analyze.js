...
function classifyMood(features) {
  const avgDanceability = features.reduce((s, f) => s + (f.danceability ?? 0), 0) / features.length;
  const avgMood = features.reduce((s, f) => s + (f.mood ?? 0), 0) / features.length;
  const avgTempo = features.reduce((s, f) => s + (f.tempo ?? 0), 0) / features.length;
...
router.get("/analyze", async (req, res) => {
  ...
  try {
    ...
    const features = [];
    for (const item of playlistData.items.slice(0, 20)) { // limit for speed
